...
test('legacy auto-submit env stays queue-only and never reaches the TTY injector', async () => {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'cdc-home-'));
  const config = loadConfig({
    HOME: home,
    DISCORD_INSTANCE: 'test',
    CODEX_DISCORD_DELIVERY_MODE: 'tty',
    CODEX_DISCORD_TTY_AUTO_SUBMIT_COMPAT: 'true',
    CODEX_DISCORD_TTY: '/dev/pts/9',
  });
  let injectorCalls = 0;
  ...
  const result = await delivery.deliver(discordMessage('m-auto', 'must remain queued'));
  const flushed = await delivery.flush();

  assert.equal(result.status, 'queued');
  assert.equal(result.reason, 'tty_auto_submit_compat_disabled');
  assert.equal(flushed.status, 'queued');
  assert.equal(flushed.reason, 'tty_auto_submit_compat_disabled');
  assert.equal(injectorCalls, 0);
  const persisted = JSON.parse(fs.readFileSync(config.paths.deliveryQueuePath, 'utf8'));
  assert.deepEqual(persisted.items.map((item) => item.normalized.messageId), ['m-auto']);
  assert.deepEqual(persisted.completed, []);
  assert.equal(persisted.blocked.reason, 'tty_auto_submit_compat_disabled');
});
